...
func (q *Query) Login(requser models.Auth , usertype string) (models.Auth , error) {
	...
	if usertype == "admin" {
		 err := q.db.QueryRow("SELECT user_id , user_name , password FROM admin where user_name=$1" , requser.Name).Scan(&user.UserId , &user.Name , &user.Password)
		 if err != nil {
			return user , nil
		 }
	}
	if usertype == "users"{
		err := q.db.QueryRow("SELECT user_id , user_name , password FROM users where user_name=$1" , requser.Name).Scan(&user.UserId , &user.Name , &user.Password)
		 if err != nil {
			return user , nil
		 }
	}
